...
export function repsForSet(exercise: Pick<Exercise, "reps" | "repsPerSet">, setIndex: number) {
  return exercise.repsPerSet?.[setIndex] ?? exercise.reps ?? 0;
}

export function resizeRepsPerSet(exercise: Pick<Exercise, "reps" | "repsPerSet">, sets: number) {
  const safeSets = Math.max(0, Math.min(20, sets));
  const fallback = exercise.repsPerSet?.at(-1) ?? exercise.reps ?? 10;
  return Array.from({ length: safeSets }, (_, index) => exercise.repsPerSet?.[index] ?? fallback);
}

export function weightForSet(exercise: Pick<Exercise, "weightKg" | "weightsPerSetKg">, setIndex: number) {
  if (Array.isArray(exercise.weightsPerSetKg) && setIndex < exercise.weightsPerSetKg.length) {
    return exercise.weightsPerSetKg[setIndex] ?? undefined;
  }
  return exercise.weightKg;
}

export function resizeWeightsPerSet(exercise: Pick<Exercise, "weightKg" | "weightsPerSetKg">, sets: number) {
  const safeSets = Math.max(0, Math.min(20, sets));
  const existing = Array.isArray(exercise.weightsPerSetKg) ? exercise.weightsPerSetKg : [];
  const fallback = existing.length ? existing.at(-1) ?? null : exercise.weightKg ?? null;
  return Array.from({ length: safeSets }, (_, index) => index < existing.length ? existing[index] ?? null : fallback);
}
...
export async function loadWorkoutHistory(): Promise<WorkoutHistoryEntry[]> { const raw = await AsyncStorage.getItem(HISTORY_KEY); if (!raw) return []; try { const parsed = JSON.parse(raw) as WorkoutHistoryEntry[]; return Array.isArray(parsed) ? parsed.sort((a, b) => b.completedAt.localeCompare(a.completedAt)) : []; } catch { return []; } }
export async function saveWorkoutHistory(entries: WorkoutHistoryEntry[]) { await AsyncStorage.setItem(HISTORY_KEY, JSON.stringify(entries)); }
...
export function emptyExercise(): Exercise { return { id: uid(), name: "", sets: 3, reps: 10, repsPerSet: [10, 10, 10], weightsPerSetKg: [null, null, null] }; }
...
export function formatDateTime(date: string) { return new Intl.DateTimeFormat("de-DE", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date(date)); }
export function exerciseSummary(exercise?: Exercise, unit: WeightUnit = "kg") { if (!exercise) return "Keine Übung"; const weights = resizeWeightsPerSet(exercise, exercise.sets); const hasWeight = weights.some((weight) => typeof weight === "number" && weight > 0); const weight = hasWeight ? ` · ${weights.map((value) => value ? Number((unit === "lbs" ? value * 2.20462 : value).toFixed(1)) : "—").join("/")} ${unit}` : ""; const reps = exercise.repsPerSet.length && new Set(exercise.repsPerSet).size > 1 ? exercise.repsPerSet.join("/") : String(repsForSet(exercise, 0)); return `${exercise.sets} Sätze · ${reps} Wdh.${weight}`; }
