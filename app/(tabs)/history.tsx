...
function SwipeWorkoutRow({ workout, index, colors, onOpen, onDelete }: { workout: Workout; index: number; colors: any; onOpen: () => void; onDelete: () => void }) {
  ...
  return <Animated.View entering={FadeInDown.delay(index * 70).duration(320)} layout={Layout.duration(250)} style={{ marginBottom: 12, position: "relative", overflow: "hidden", borderRadius: 16, backgroundColor: colors.background }}><View style={{ position: "absolute", inset: 0, alignItems: "flex-end", justifyContent: "center", paddingRight: 22 }}><IconSymbol name="trash.fill" size={22} color={colors.background} /></View><GestureDetector gesture={pan}><Animated.View style={rowStyle}><View className="flex-row items-center rounded-2xl p-4" style={{ borderWidth: 1, borderColor: colors.border }}><Pressable onPress={onOpen} style={({ pressed }) => [{ flex: 1, flexDirection: "row", alignItems: "center", opacity: pressed ? 0.65 : 1 }]}><View className="mr-4 h-12 w-12 items-center justify-center rounded-2xl bg-background"><IconSymbol name="checkmark.circle.fill" size={23} color={colors.foreground} /></View><View className="flex-1"><Text className="font-bold text-foreground">{workout.title}</Text><Text className="mt-1 text-sm text-muted">{formatDate(workout.archivedAt!)} · {workout.exercises.length} Übungen</Text><Text numberOfLines={1} className="mt-1 text-sm text-muted">{exerciseSummary(workout.exercises[0])}</Text></View></Pressable><Pressable onPress={onDelete} style={({ pressed }) => [{ padding: 10, opacity: pressed ? 0.5 : 1 }]}><IconSymbol name="trash.fill" size={20} color={colors.error} /></Pressable></View></Animated.View></GestureDetector></Animated.View>;
}
