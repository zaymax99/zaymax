...
export default function WorkoutEditorScreen() {
  ...
  useEffect(() => {
    if (isNew && (title === "Mein Workout" || title === "My Workout")) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!isNew && id) {
        const existing = (await loadWorkouts()).find((item) => item.id === id);
        if (existing) {
          ...
          <View className="mt-5 flex-row items-center justify-between">
            <View>
              ...
              <Text className="mt-1 text-sm text-muted">
                {t("Gedrückt halten und verschieben", "Press, hold and move")}
              </Text>
            </View>
...
function ExerciseCard({
  ...
  function moveDrag(target: number) {
    if (target !== lastSlot.current && Platform.OS !== "web") {
      void Haptics.selectionAsync();
      lastSlot.current = target;
    }
    onDragMove(target);
  }
  function endDrag(target: number) {
    if (Platform.OS !== "web")
    ...
    .onUpdate((event) => {
      ...
      const shift =
        event.translationY > 0
          ? Math.ceil(event.translationY / 120)
          : Math.floor(event.translationY / 120);
      runOnJS(moveDrag)(
        Math.max(0, Math.min(total, index + shift + (shift > 0 ? 1 : 0))),
      );
    })
    .onEnd((event) => {
      const shift =
  ...
  return (
    <GestureDetector gesture={gesture}>
      ...
      >
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center">
            <IconSymbol
              ...
              color={colors.muted}
            />
            <Text className="ml-2 text-xs font-black uppercase tracking-[2px] text-muted">
        ...
        <Text className="mt-5 text-xs font-black uppercase tracking-[1px] text-muted">
...
function NumberField({
  ...
  return (
    <View className="flex-1">
      <Text className="mb-2 text-xs font-bold uppercase tracking-[1px] text-muted">
        {label}
      </Text>
      <TextInput
